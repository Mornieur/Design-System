...
test('captures the current button page baseline', async ({
  page
}, testInfo) => {
  test.skip(
  ...
  await stabilizeDocsPage(page, '/components/button', 'dark');
  await page.evaluate(() => window.scrollTo(0, 0));

  await expect(page).toHaveScreenshot('current-button-dark-desktop.png', {
    animations: 'disabled',
    caret: 'hide',
    fullPage: true
  });
